...
// 	$this->db->distinct('bank_add.bank_id');
	//BANK LIST
...
// 	}
    ...
    public function get_bank_list()
    {
        ...
        $this->db->from('bank_add');
        $this->db->join('bank_summary', 'bank_summary.bank_id = bank_add.bank_id', 'left');
        $this->db->group_by('bank_add.bank_id, bank_add.bank_name, bank_add.ac_name, bank_add.ac_number, bank_add.branch, bank_add.signature_pic, bank_add.status'); // Include all non-aggregated columns from bank_add table in the GROUP BY clause
        $this->db->where('bank_add.status', 1);
