...
public function return_invoice_entry()
	...
	{
	    $this->db->distinct('a.invoice_id');
        $this->db->select('a.*, b.customer_name');
        $this->db->from('product_return a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
        $this->db->where('usablity', 1);
        // $this->db->group_by('a.invoice_id'); // Adjusted the GROUP BY clause
        $this->db->order_by('a.invoice_id', 'desc'); // Added order_by clause if needed
        $this->db->limit(500);
        $query = $this->db->get();
