...
class reports extends CI_Model {
  ...
  $this->db->select(" DISTINCT (a.product_id), b.manufacturer_name,a.product_name,a.generic_name,a.strength,((select ifnull(sum(quantity),0) from product_purchase_details where product_id= `a`.`product_id`)-(select ifnull(sum(quantity),0) from invoice_details where product_id= `a`.`product_id`)) as 'stock'");
       $this->db->from('product_information a');
       $this->db->join('manufacturer_information b','b.manufacturer_id=a.manufacturer_id','left');
        //  $this->db->having('stock < 10');
        //  $this->db->group_by('a.product_id');
	...
	public function out_of_date_count(){
		  ...
		  $date=date('Y-m-d');
         $this->db->select("DISTINCT (a.product_id),b.*,a.product_name,a.strength,((select ifnull(sum(quantity),0) from product_purchase_details where product_id= `a`.`product_id`)-(select ifnull(sum(quantity),0) from invoice_details where product_id= `a`.`product_id`)) as 'stock'");
         $this->db->from('product_information a');
         $this->db->join('product_purchase_details b','b.product_id=a.product_id','left');
         $this->db->where('b.expeire_date <=', $date);
        //  $this->db->having('stock > 0');
...
public function stock_report_batch_bydate($perpage,$page){
	...
	public function getCheckBatchStock($postData=null){
         ...
         ## Fetch records
         $this->db->distinct('a.batch_id,a.product_id');
         $this->db->select("a.*,
				m.product_name,
				m.strength,
				");
